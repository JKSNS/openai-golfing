...
class Hyperparameters:
 ...
 qk_gain_init = float(os.environ.get("QK_GAIN_INIT", 5.0))  # sharper attention (was 4.0 in PR #1278)
 ...
 muon_wd = float(os.environ.get("MUON_WD", 0.09))  # higher WD (was 0.04 in PR #1278)
 adam_wd = float(os.environ.get("ADAM_WD", 0.09))
 ...
 slot_steps = int(os.environ.get("SLOT_STEPS", 32))  # SLOT-32 (matches PR #1278 winning config)
 slot_lr = float(os.environ.get("SLOT_LR", 0.015))  # higher LR (was 0.008)
 ...
 recur_layers = os.environ.get("RECUR_LAYERS", "4,5")  # depth recurrence enabled by default
 muon_eq_r = bool(int(os.environ.get("MUON_EQ_R", "1")))  # row-normalize before NS5
 # Pre-quant TTT
...
def ttt_adapt_adamw(args, base_model, device, val_tokens, rank=0, world_size=1, log0=print):
 ...
 log0(f"ttt:trainable={sum(p.numel() for p in ttt_params)} frozen={sum(p.numel() for p in base_model.parameters() if not p.requires_grad)}")
 optimizer = torch.optim.AdamW(ttt_params, lr=args.ttt_lr, weight_decay=0.0)
 scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=args.ttt_epochs, eta_min=args.ttt_lr * 0.1)
 ...
 for epoch in range(args.ttt_epochs):
  ...
  for bs in range(my_start, my_end, batch_seqs):
   ...
   raw_start, raw_end = bs * seq_len, be * seq_len + 1
  ...
  scheduler.step()
  log0(f"ttt:epoch {epoch+1}/{args.ttt_epochs} loss:{epoch_loss_sum.item()/max(epoch_tokens.item(),1):.4f} t={time.perf_counter()-t0:.1f}s")
 for p in base_model.parameters(): p.requires_grad_(True)
 base_model.eval()
 log0(f"ttt:done elapsed={time.perf_counter()-t0:.1f}s")
...
def main() -> None:
 ...
 # Pre-Quant TTT: fine-tune on val data BEFORE quantization
 if args.ttt_enabled:
  log0(f"ttt:start lr={args.ttt_lr} epochs={args.ttt_epochs} freeze_blocks={args.ttt_freeze_blocks}")
  t_ttt = time.perf_counter()
  ttt_adapt_adamw(args, base_model, device, val_tokens, rank=rank, world_size=world_size, log0=log0)
