...
def eval_val_ttt(h,device,val_data,base_model,batch_seqs=32):
	...
	if h.ttt_ema_teacher_lambda>0:
		teacher_model=copy.deepcopy(base_model);teacher_model.eval()
		for p in teacher_model.parameters():p.requires_grad_(False)
		log(f"ttt:ema_teacher lambda={h.ttt_ema_teacher_lambda} decay={h.ttt_ema_decay}")
	if h.ttt_anchor_lambda>0:log(f"ttt:anchor lambda={h.ttt_anchor_lambda}")
					...
					for bs in range(0,my_chunk_seqs,batch_seqs):
						...
						if h.ttt_confidence_weighted or h.ttt_ema_teacher_lambda>0:
							if h.ttt_confidence_weighted:
								with torch.no_grad(),torch.autocast(device_type='cuda',dtype=torch.bfloat16):logits_eval=base_model.forward_logits(x);nll_eval=F.cross_entropy(logits_eval.reshape(-1,logits_eval.size(-1)).float(),y.reshape(-1),reduction='none').reshape_as(y);cw_weights=(1.-torch.exp(-nll_eval)).clamp(min=h.ttt_weight_floor)
							if h.ttt_ema_teacher_lambda>0:
								with torch.no_grad(),torch.autocast(device_type='cuda',dtype=torch.bfloat16):teacher_logits=teacher_model.forward_logits(x);teacher_log_probs=F.log_softmax(teacher_logits.float(),dim=-1)
							with torch.autocast(device_type='cuda',dtype=torch.bfloat16):
								logits_train=base_model.forward_logits(x)
								nll_train=F.cross_entropy(logits_train.reshape(-1,logits_train.size(-1)).float(),y.reshape(-1),reduction='none').reshape_as(y)
								...
								if h.ttt_ema_teacher_lambda>0:
									...
									kl=F.kl_div(student_log_probs.reshape(-1,student_log_probs.size(-1)),teacher_log_probs.reshape(-1,teacher_log_probs.size(-1)),reduction='batchmean',log_target=True)
						...
						if h.ttt_ema_teacher_lambda>0:
							with torch.no_grad():
								for tp,sp in zip(teacher_model.parameters(),ttt_params):tp.data.mul_(h.ttt_ema_decay).add_(sp.detach(),alpha=1.-h.ttt_ema_decay)
	if dist.is_available()and dist.is_initialized():dist.all_reduce(loss_sum,op=dist.ReduceOp.SUM);dist.all_reduce(token_count,op=dist.ReduceOp.SUM);dist.all_reduce(byte_count,op=dist.ReduceOp.SUM)
